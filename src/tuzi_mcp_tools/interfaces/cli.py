...
def generate(
    ...
    quality: str = typer.Option("auto", "--quality", "-q", help="Image quality: auto, low, medium, high"),
    size: str = typer.Option("auto", "--size", "-s", help="Image size: auto, 1024x1024, 1536x1024, 1024x1536"),
    format: str = typer.Option("png", "--format", "-f", help="Output format: png, jpeg, webp"),
    background: str = typer.Option("opaque", "--background", "-b", help="Background type: opaque, transparent"),
    compression: Optional[int] = typer.Option(None, "--compression", "-c", help="Compression level (0-100) for JPEG/WebP"),
    conversation_id: Optional[str] = typer.Option(None, "--conversation-id", help="Continue existing conversation"),
    input_image: Optional[str] = typer.Option(None, "--input-image", help="Path to reference image"),
):
    ...
    try:
        ...
        if result.success:
            console.print(f"[green]✓[/green] Image generated successfully: {result.output_path}")
        else:
...
def flux(
    ...
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Output file path (default: auto-generated)"),
    aspect_ratio: str = typer.Option("1:1", "--aspect-ratio", "-a", help="Aspect ratio: 1:1, 16:9, 9:16, 4:3, 3:4, 21:9, 9:21"),
    format: str = typer.Option("png", "--format", "-f", help="Output format: png, jpg, jpeg, webp"),
    seed: Optional[int] = typer.Option(None, "--seed", help="Seed for reproducible generation"),
    conversation_id: Optional[str] = typer.Option(None, "--conversation-id", help="Continue existing conversation"),
    input_image: Optional[str] = typer.Option(None, "--input-image", help="Path to reference image"),
):
    ...
    try:
        ...
        if result.success:
            console.print(f"[green]✓[/green] FLUX image generated successfully: {result.output_path}")
        else:
...
def survey(
    ...
    conversation_id: Optional[str] = typer.Option(None, "--conversation-id", help="Continue existing conversation"),
    enable_web_search: bool = typer.Option(True, "--web-search/--no-web-search", help="Enable web search capabilities"),
):
...
def conversations():
    ...
    try:
        ...
        for conv in conversations:
            ...
            last_time = last_message.timestamp.strftime("%Y-%m-%d %H:%M") if last_message else "Unknown"
            
            console.print(f"  [cyan]{conv.id}[/cyan] ({conv.type.value}) - {message_count} messages - Last: {last_time}")
