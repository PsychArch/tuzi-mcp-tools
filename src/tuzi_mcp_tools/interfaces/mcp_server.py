...
async def submit_gpt_image(
    ...
    output_path: Annotated[str, Field(description="Absolute path where to save the generated image")],
    quality: Annotated[
    ...
    conversation_id: Annotated[
        Optional[str], 
        Field(description="Conversation ID to continue an existing conversation (optional)")
    ] = None,
    input_image: Annotated[
        Optional[str], 
        Field(description="Base64 encoded reference image (optional)")
    ] = None
) -> str:
...
async def task_barrier(
    output_dir: Annotated[str, Field(description="Directory where to save all generated images")] = "images"
) -> str:
    ...
    try:
        ...
        for task_id, task_result in results["results"].items():
            if task_result["success"]:
                output_lines.append(f"{task_id}: success")
            else:
                error_msg = task_result["error"] or "Unknown error"
                output_lines.append(f"{task_id}: failed - {error_msg}")
