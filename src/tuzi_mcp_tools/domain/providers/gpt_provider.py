...
from ...infrastructure.api_client import TuZiApiClient, ApiConfig, StreamProcessor
from ...infrastructure.file_manager import ImageUrlExtractor
from ...infrastructure.progress_reporter import StreamProgressTracker, ModelFallbackTracker
...
class GptImageRequest(ImageGenerationRequest):
    ...
    def validate(self) -> None:
        ...
        if self.size not in valid_sizes:
            raise ValueError(f"Invalid size: {self.size}. Must be one of: {', '.join(valid_sizes)}")
        
        # Validate compression
        if self.output_compression is not None:
            if not (0 <= self.output_compression <= 100):
                raise ValueError(f"Invalid compression: {self.output_compression}. Must be between 0 and 100")
        
        # Validate background transparency only works with PNG/WebP
        if self.background == BackgroundType.TRANSPARENT and self.format not in [ImageFormat.PNG, ImageFormat.WEBP]:
...
class GptImageProvider(StreamingImageProvider, ConversationalImageProvider):
    ...
    async def generate_async(self, request: ImageGenerationRequest) -> ImageGenerationResult:
    ...
    def generate_streaming_sync(
        ...
        for model in GPT_MODEL_FALLBACK_ORDER:
            try:
                ...
                # Make API request
                response = self.api_client.post_sync("chat/completions", data, stream=True)
    ...
    async def generate_streaming_async(
        ...
        for model in GPT_MODEL_FALLBACK_ORDER:
            try:
                ...
                stream_result = await self.api_client.post_async_with_stream_processing("chat/completions", data)
    ...
    def _convert_to_gpt_request(self, request: ImageGenerationRequest) -> GptImageRequest:
    ...
    def _generate_non_streaming_sync(self, request: GptImageRequest) -> ImageGenerationResult:
    ...
    async def _generate_non_streaming_async(self, request: GptImageRequest) -> ImageGenerationResult:
